...
-- Simple Query: Retrieve all packages with a price less than 30,000
SELECT *
FROM package
WHERE Packprice < 30000;
-- Simple Query: Retrieve all categories
SELECT *
FROM category;
-- Simple Query: Retrieve the details of packages in the 'Religious Tours' category
SELECT *
FROM package
WHERE Category = (
    SELECT Cat_id
    FROM category
    WHERE Cat_name = 'Religious Tours'
);
...
-- Trigger`
DELIMITER //
CREATE TRIGGER set_enquiry_status
BEFORE INSERT ON enquiry
FOR EACH ROW
BEGIN
    IF NEW.Adults > 2 THEN
        SET NEW.Statusfield = 'Pending';
    ELSE
        SET NEW.Statusfield = 'Confirmed';
    END IF;
END;
//
DELIMITER ;
...
WHERE Category = (
    ...
    WHERE Cat_name = 'Adventure Tours'
)
AND Packprice < 50000;
...
-- --------------------------------------------------------
-- Correlated Query to retrieve packages with price less than 50,000 in the 'Adventure Tours' category
--
SELECT *
FROM package p
WHERE p.Category = (
    SELECT Cat_id
    FROM category c
    WHERE c.Cat_name = 'Family Tours'
)
AND p.Packprice < 100000;

-- Delete a record from the category table
DELETE FROM `category` WHERE `Cat_id` = 7;
COMMIT;

-- Delete a record from the enquiry table
DELETE FROM `enquiry` WHERE `Enquiryid` = 10;
COMMIT;

--

-- Add this stored procedure to the end of your SQL dump

-- Stored procedure to update package prices by a percentage
DELIMITER //
CREATE PROCEDURE UpdatePackagePrices(IN percentage FLOAT)
BEGIN
    UPDATE package
    SET Packprice = Packprice * (1 + percentage / 200);
END;
//
DELIMITER ;
-- Example of calling the stored procedure to increase prices by 10%
CALL UpdatePackagePrices(20);
...
BEGIN
    UPDATE package
    SET Packprice = Packprice * (1 + percentage / 100);
END;
//
DELIMITER ;
-- Example of calling the stored procedure to increase prices by 10%
CALL UpdatePackagePrices(10);
